...
#define INTERNAL_CATCH_TEST( expr, isNot, stopOnFailure, macroName ) \
    do{ try{ \
        INTERNAL_CATCH_ACCEPT_EXPR( ( Catch::ResultBuilder( __FILE__, __LINE__, macroName, #expr, isNot )->*static_cast<bool>( expr ) ), stopOnFailure ); \
        if( Catch::isTrue( false ) ){ bool internal_catch_dummyResult = static_cast<bool>( expr ); Catch::isTrue( internal_catch_dummyResult ); } \
